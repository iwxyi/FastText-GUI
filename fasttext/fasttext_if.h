...
public:
    ...
    static void quantize(const std::vector<std::string>& args) {
        Args a = Args();
        if (args.size() < 3) {
            printQuantizeUsage();
            a.printHelp();
    ...
    static void printWordVectors(const std::vector<std::string> args) {
        ...
        // mrxy: 添加文件输入
        std::ifstream ifs;
        ifs.open(args[args.size()-1]);
        if (!ifs.is_open()) {
            std::cerr << "Input file cannot be opened!" << std::endl;
            return ;
        }
        while (!ifs.eof() && ifs >> word) {
            fasttext.getWordVector(vec, word);
            std::cout << word << " " << vec << std::endl;
        }
        ifs.close();
    ...
    static void printSentenceVectors(const std::vector<std::string> args) {
        if (args.size() != 3+1) {
    ...
    static void printNgrams(const std::vector<std::string> args) {
        if (args.size() != 4) {
    ...
    static void nn(const std::vector<std::string> args) {
        int32_t k;
        if (args.size() == 3+1) {
            k = 10;
        } else if (args.size() == 4+1) {
            k = std::stoi(args[3]);
        } else {
            printNNUsage();
            return ;
    ...
    static void analogies(const std::vector<std::string> args) {
        int32_t k;
        if (args.size() == 3+1) {
            k = 10;
        } else if (args.size() == 4+1) {
    ...
    static void dump(const std::vector<std::string>& args) {
        if (args.size() < 4) {
            printDumpUsage();
            return ;
        }
    ...
    static int entrance(std::vector<std::string> args) {
        if (args.size() < 2) {
            printUsage();
            return EXIT_FAILURE;
        }
